...
- **Communication** — Web UI with command-palette agent prompts and Markdown copy/export, Telegram, WhatsApp with opt-in bounded document ingest, Signal, Microsoft Teams, Discord, Slack with native live tool cards, Nostr DMs + NIP-29 group chat (incl. Block's Buzz channels), API access, voice I/O (8 TTS + 7 STT providers), mobile PWA with push notifications
