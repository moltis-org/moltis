...
function markTmuxInstallPromptSeen(): void {
	...
	try { if (typeof localStorage !== "undefined") localStorage.setItem(TmuxInstallPromptStorageKey, "1"); } catch { /* private mode */ }
}

function clearObservers(): void {
	if (resizeObserver) { resizeObserver.disconnect(); resizeObserver = null; }
	if (themeObserver) { themeObserver.disconnect(); themeObserver = null; }
	if (windowResizeListener) { window.removeEventListener("resize", windowResizeListener); windowResizeListener = null; }
	if (fontsReadyListener && typeof document !== "undefined" && document.fonts?.removeEventListener) {
		document.fonts.removeEventListener("loadingdone", fontsReadyListener); fontsReadyListener = null;
	}
}

function clearScheduledFit(): void { if (fitRaf) { cancelAnimationFrame(fitRaf); fitRaf = 0; } }
function clearReconnectTimer(): void { if (reconnectTimer) { clearTimeout(reconnectTimer); reconnectTimer = null; } }
function clearResizeSettleTimers(): void { for (const t of resizeSettleTimers) clearTimeout(t); resizeSettleTimers = []; }
function clearInputQueue(): void { if (inputFlushTimer) { clearTimeout(inputFlushTimer); inputFlushTimer = null; } pendingInput = ""; }
function clearWindowsRefreshTimer(): void { if (windowsRefreshTimer) { clearInterval(windowsRefreshTimer); windowsRefreshTimer = null; } }
...
function windowLabel(w: WindowInfo): string { const title = w.name?.trim() || "shell"; return `${w.index}: ${title}`; }

function renderWindowTabs(): void {
	if (!tabsEl) return;
	while (tabsEl.firstChild) tabsEl.removeChild(tabsEl.firstChild);
	if (!tmuxPersistenceEnabled) {
		const s = document.createElement("span"); s.className = "terminal-tab-empty"; s.textContent = "tmux unavailable"; tabsEl.appendChild(s); return;
	}
	if (!terminalWindows.length) {
		const s = document.createElement("span"); s.className = "terminal-tab-empty"; s.textContent = "No tmux windows"; tabsEl.appendChild(s); return;
	}
	for (const w of terminalWindows) {
		const tab = document.createElement("button"); tab.type = "button"; tab.className = "terminal-tab";
		if (w.id === activeWindowId) tab.classList.add("active");
		tab.title = `Attach ${windowLabel(w)}`; tab.textContent = windowLabel(w);
		tab.addEventListener("click", () => onWindowTabClick(w.id));
		tabsEl.appendChild(tab);
	}
}

function chooseActiveWindow(windows: WindowInfo[], preferred: string | null, payloadActive: string | null): string | null {
	if (!windows.length) return null;
	for (const c of [preferred, payloadActive, activeWindowId]) { if (c && windows.some((w) => w.id === c)) return c; }
...
function applyWindowsState(payload: WindowsPayload, preferred: string | null): void {
	...
	for (const raw of Array.isArray(payload?.windows) ? payload.windows : []) { const p = normalizeWindowPayload(raw); if (p) next.push(p); }
	next.sort((a, b) => a.index - b.index);
	terminalWindows = next;
	const pa = typeof payload?.activeWindowId === "string" && payload.activeWindowId.trim() ? payload.activeWindowId.trim() : null;
...
async function fetchTerminalWindows(): Promise<WindowsPayload> {
	...
	let p: WindowsPayload; try { p = await r.json(); } catch { p = {}; }
...
async function refreshTerminalWindows(opts?: { preferredWindowId?: string | null; silent?: boolean }): Promise<void> {
	...
	} catch (e) { if (!opts?.silent) setStatus((e as Error)?.message || "Failed to refresh terminal windows", "error"); }
}

function startWindowsRefreshLoop(): void {
	clearWindowsRefreshTimer();
	if (!tmuxPersistenceEnabled) return;
	windowsRefreshTimer = setInterval(() => { void refreshTerminalWindows({ silent: true }); }, WINDOW_REFRESH_MS);
}
...
function scheduleFit(forceResize?: boolean): void {
	...
	fitRaf = requestAnimationFrame(() => {
		fitRaf = 0; if (!fitAddon) return;
		try { fitAddon.fit(); sendResizeIfChanged(fr); } catch { /* transient layout */ }
	});
}
...
function kickResizeSettleLoop(): void {
	...
	for (const d of [0, 50, 160, 380, 800]) {
		resizeSettleTimers.push(setTimeout(() => { if (xterm) { scheduleFit(true); sendResizeIfChanged(true); } }, d));
	}
}

function requestWindowSwitch(windowId: string): boolean {
	if (!tmuxPersistenceEnabled || !windowId) return false;
	pendingWindowId = windowId; activeWindowId = windowId;
	renderWindowTabs(); setStatus("Switching tmux window...", "ok");
	return socket?.readyState === WebSocket.OPEN && sendSocketMessage({ type: "switch_window", window: windowId }) || false;
}
...
function handleActiveWindowEvent(payload: TerminalMessage): void {
	...
	if (!wid) return;
	activeWindowId = wid; pendingWindowId = null;
	renderWindowTabs(); setStatus("Switched tmux window.", "ok");
	startWindowsRefreshLoop(); kickResizeSettleLoop();
...
function onWindowTabClick(windowId: string): void {
	...
	terminalAvailable = false; setControlsEnabled(false);
...
async function createTerminalWindow(): Promise<void> {
	...
	creatingWindow = true; setWindowControlsEnabled(); setStatus("Creating tmux window...", "ok");
	try {
		const r = await fetch("/api/terminal/windows", { method: "POST", headers: { Accept: "application/json", "Content-Type": "application/json" }, body: JSON.stringify({}) });
		let p: WindowsPayload; try { p = await r.json(); } catch { p = {}; }
		...
		if (Array.isArray(p?.windows)) { tmuxPersistenceEnabled = true; applyWindowsState(p, cid); }
		else await refreshTerminalWindows({ preferredWindowId: cid, silent: true });
		if (cid && activeWindowId !== cid) { if (!requestWindowSwitch(cid)) { if (xterm) xterm.reset(); connectTerminalSocket(); } }
		else { if (xterm) xterm.reset(); connectTerminalSocket(); }
		setStatus("Created tmux window.", "ok");
	} catch (e) { setStatus((e as Error)?.message || "Failed to create tmux window", "error"); }
	finally { creatingWindow = false; setWindowControlsEnabled(); }
}
...
function buildXtermTheme(): Record<string, string> {
	return {
		background: getCssVar("--bg", "#0f1115"), foreground: getCssVar("--text", "#e4e4e7"),
		cursor: getCssVar("--accent", "#4ade80"), cursorAccent: getCssVar("--bg", "#0f1115"),
		selectionBackground: getCssVar("--accent-subtle", "#4ade801f"),
	};
}

function applyTheme(): void { if (xterm) xterm.options.theme = buildXtermTheme(); }
...
function clearOscStabilityGuards(): void {
	for (const d of oscHandlerDisposables) { try { d.dispose(); } catch { /* ignore */ } }
...
function queueInput(data: string): void {
	...
	pendingInput += data;
	if (!inputFlushTimer) inputFlushTimer = setTimeout(() => { inputFlushTimer = null; flushInputQueue(); }, INPUT_FLUSH_MS);
}
...
function flushInputQueue(): void {
	...
	if (pendingInput.length > 0 && !inputFlushTimer) inputFlushTimer = setTimeout(() => { inputFlushTimer = null; flushInputQueue(); }, INPUT_FLUSH_MS);
}
...
async function initXterm(): Promise<void> {
	...
	xterm = new TerminalCtorRef({ convertEol: false, disableStdin: false, cursorBlink: true, scrollback: 4000, fontFamily: "JetBrains Mono, ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace", fontSize: 12, lineHeight: 1.35, theme: buildXtermTheme() });
	...
	xtermResizeDisposable = xterm.onResize((sz: { cols: number; rows: number }) => { updateSizeIndicator(sz.cols, sz.rows); sendResizeIfChanged(); });
	scheduleFit();
	terminalEl.addEventListener("click", () => { if (xterm) xterm.focus(); });
	if (typeof ResizeObserver !== "undefined") { resizeObserver = new ResizeObserver(() => scheduleFit()); resizeObserver.observe(terminalEl.parentElement || terminalEl); }
	if (typeof window !== "undefined") { windowResizeListener = () => scheduleFit(); window.addEventListener("resize", windowResizeListener); }
	if (typeof document !== "undefined" && document.fonts?.ready && typeof document.fonts.ready.then === "function") document.fonts.ready.then(() => scheduleFit());
	if (typeof document !== "undefined" && document.fonts?.addEventListener) { fontsReadyListener = () => scheduleFit(); document.fonts.addEventListener("loadingdone", fontsReadyListener); }
	themeObserver = new MutationObserver(() => applyTheme());
	themeObserver.observe(document.documentElement, { attributes: true, attributeFilter: ["data-theme"] });
}

function disposeXterm(): void {
	clearObservers(); clearScheduledFit(); clearResizeSettleTimers(); clearOscStabilityGuards();
	if (xtermDataDisposable) { xtermDataDisposable.dispose(); xtermDataDisposable = null; }
	if (xtermResizeDisposable) { xtermResizeDisposable.dispose(); xtermResizeDisposable = null; }
	if (xterm) { xterm.dispose(); xterm = null; }
	fitAddon = null; lastSentCols = 0; lastSentRows = 0; updateSizeIndicator(0, 0);
}
...
function decodeBase64ToBytes(encoded: string): Uint8Array | null {
	...
	try { const bin = atob(encoded); const b = new Uint8Array(bin.length); for (let i = 0; i < bin.length; i++) b[i] = bin.charCodeAt(i) & 0xff; return b; } catch { return null; }
}

function writeToXterm(chunk: string | Uint8Array, scrollBottom: boolean): void {
	if (!xterm) return;
	if ((typeof chunk === "string" && !chunk.length) || (chunk instanceof Uint8Array && !chunk.length)) { if (scrollBottom) xterm.scrollToBottom(); return; }
	xterm.write(chunk, () => { if (scrollBottom && xterm) xterm.scrollToBottom(); });
}
...
function scheduleReconnect(): void {
	...
	reconnectTimer = setTimeout(() => { reconnectTimer = null; connectTerminalSocket(); }, RECONNECT_DELAY_MS);
}
...
function applyReadyPayload(payload: ReadyPayload): void {
	...
	if (metaEl) metaEl.textContent = terminalAvailable ? (pe ? `Persistent tmux session, user ${payload.user || "unknown"}` : `Ephemeral host shell, user ${payload.user || "unknown"}`) : "Host shell unavailable";
	if (hintEl) {
		if (!terminalAvailable) hintEl.textContent = "Unable to open host shell.";
		else if (pe) hintEl.textContent = "Interactive host shell with persistent tmux session. Click inside terminal and type commands directly.";
		else if (pa) hintEl.textContent = "Interactive host shell (ephemeral). Enable tmux persistence from terminal settings when available.";
		else if (ic) hintEl.textContent = first ? "First connection tip: run the install command once to enable persistent tmux sessions." : `Interactive host shell (ephemeral). Install tmux for persistence: ${ic}`;
	...
	renderWindowTabs(); setWindowControlsEnabled();
	if (terminalAvailable) {
		kickResizeSettleLoop(); updateSizeIndicator(xterm?.cols || 0, xterm?.rows || 0);
		if (pe) { setStatus("Connected to host shell with persistent tmux session.", "ok"); startWindowsRefreshLoop(); void refreshTerminalWindows({ preferredWindowId: activeWindowId, silent: true }); }
		else { setStatus("Connected to host shell (ephemeral session).", "ok"); clearWindowsRefreshTimer(); }
		flushInputQueue(); if (xterm) xterm.focus();
	} else { clearWindowsRefreshTimer(); updateSizeIndicator(0, 0); setStatus("Failed to open host shell.", "error"); }
}

function handleTerminalMessage(payload: TerminalMessage): void {
	if (!(payload && typeof payload === "object")) return;
	switch (payload.type) {
		case "ready": applyReadyPayload(payload as unknown as ReadyPayload); break;
		case "active_window": handleActiveWindowEvent(payload); break;
		case "output":
			if (payload.encoding === "base64") { const b = decodeBase64ToBytes(payload.data || ""); if (b) appendOutputChunk(b, false); }
			else appendOutputChunk(payload.data || "", false);
			break;
		case "status": setStatus(payload.text || "", payload.level || ""); break;
		case "error": setStatus(payload.error || "Terminal error", "error"); break;
		case "pong": break;
		default: break;
	}
}
...
function connectTerminalSocket(): void {
	if (typeof WebSocket === "undefined") { setStatus("WebSocket not supported in this browser", "error"); return; }
	clearReconnectTimer(); clearResizeSettleTimers(); closeTerminalSocket(); lastSentCols = 0; lastSentRows = 0;
	...
	socket.onmessage = (ev: MessageEvent) => { let p: TerminalMessage | null = null; try { p = JSON.parse(ev.data as string); } catch { return; } if (p) handleTerminalMessage(p); };
	socket.onerror = () => { /* onclose handles */ };
	socket.onclose = () => {
		socket = null; setControlsEnabled(false); terminalAvailable = false; clearWindowsRefreshTimer(); setWindowControlsEnabled();
		if (shuttingDown) return;
		setStatus("Terminal disconnected. Reconnecting...", "error"); scheduleReconnect();
	};
}

function sendControl(action: string): void { if (terminalAvailable) sendSocketMessage({ type: "control", action }); }

function bindEvents(): void {
	if (newTabBtn) newTabBtn.addEventListener("click", () => { void createTerminalWindow(); });
	...
	if (installTmuxBtn) installTmuxBtn.addEventListener("click", () => {
		if (!(terminalAvailable && tmuxInstallCommand)) return;
		if (!sendSocketMessage({ type: "input", data: `${tmuxInstallCommand}\n` })) { setStatus("Failed to queue install command.", "error"); return; }
		setStatus(`Queued install command: ${tmuxInstallCommand}`, "ok"); if (xterm) xterm.focus();
	});
	if (copyInstallBtn) copyInstallBtn.addEventListener("click", async () => {
		if (!tmuxInstallCommand) return;
		if (!navigator.clipboard?.writeText) { setStatus("Clipboard API unavailable in this browser.", "error"); return; }
		try { await navigator.clipboard.writeText(tmuxInstallCommand); setStatus("Install command copied to clipboard.", "ok"); } catch { setStatus("Failed to copy install command.", "error"); }
	});
}
...
export function teardownTerminal(): void {
	shuttingDown = true;
	clearReconnectTimer(); clearResizeSettleTimers(); closeTerminalSocket(); clearInputQueue(); clearWindowsRefreshTimer(); disposeXterm();
	if (_container) while (_container.firstChild) _container.removeChild(_container.firstChild);
	_container = null; terminalEl = null; metaEl = null; statusEl = null; hintEl = null; hintActionsEl = null;
	installCommandEl = null; sizeEl = null; tabsEl = null; newTabBtn = null; ctrlCBtn = null; clearBtn = null;
	restartBtn = null; installTmuxBtn = null; copyInstallBtn = null;
	terminalAvailable = false; tmuxPersistenceEnabled = false; terminalWindows = []; activeWindowId = null;
	pendingWindowId = null; creatingWindow = false; tmuxInstallCommand = "";
}
