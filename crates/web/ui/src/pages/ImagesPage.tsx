...
function ContainerRow({ container: c, sandboxAvailable }: { container: ContainerInfo; sandboxAvailable: boolean }): VNode {
	...
	return (
		<div className="provider-item flex-col gap-1 mb-1" style={{ alignItems: "stretch" }}>
			<div className="flex items-center justify-between gap-2 w-full min-w-0">
				<span className="font-mono text-xs truncate flex-1 text-[var(--text-strong)]" title={c.name}>{truncateHash(c.name)}</span>
			...
			<div className="flex items-center justify-between gap-2 w-full">
				<div className="flex items-center gap-2 text-xs text-[var(--muted)]">
					...
					<span className="font-mono" title={c.image}>{truncateHash(c.image)}</span>
					{resources.length > 0 && <span>{resources.join(" \u00b7 ")}</span>}
				</div>
				<div className="flex items-center gap-1">
					{isRunning && (
						<button className="text-xs px-2 py-0.5 rounded border border-[var(--border)] bg-transparent text-[var(--muted)] hover:text-[var(--text)] hover:border-[var(--border-strong)] transition-colors cursor-pointer"
							onClick={() => stopContainer(c.name)}
							disabled={!sandboxAvailable}
							title={sandboxAvailable ? "Stop container" : SANDBOX_DISABLED_HINT}>Stop</button>
					)}
					<button className="text-xs text-white border border-[var(--error)] px-2 py-0.5 rounded bg-[var(--error)] hover:opacity-80 transition-colors cursor-pointer"
...
function DiskUsageBar(): VNode | null {
	...
	return (
		<div className="text-xs text-[var(--muted)] flex flex-wrap gap-x-4 gap-y-1 mt-1 mb-2">
			<span>Containers: {du.containers_total} total, {du.containers_active} active &middot; {formatBytes(du.containers_size_bytes)} ({formatBytes(du.containers_reclaimable_bytes)} reclaimable)</span>
			<span>Images: {du.images_total} total, {du.images_active} active &middot; {formatBytes(du.images_size_bytes)}</span>
		</div>
	);
}
	...
	return (
		<div className="max-w-form">
			<div className="flex items-center gap-3 mb-2">
				<h3 className="text-sm font-medium text-[var(--text-strong)]">Running Containers{list.length > 0 ? ` (${list.length})` : ""}</h3>
				<button className="text-xs text-[var(--muted)] border border-[var(--border)] px-2 py-0.5 rounded-md hover:text-[var(--text)] hover:border-[var(--border-strong)] transition-colors cursor-pointer bg-transparent"
				...
				<button className="text-xs text-[var(--muted)] border border-[var(--border)] px-2 py-0.5 rounded-md hover:text-[var(--text)] hover:border-[var(--border-strong)] transition-colors cursor-pointer bg-transparent"
					onClick={() => { fetchContainers(); fetchDiskUsage(); }}
				...
				{list.length > 0 && (
					<button className="text-xs text-white border border-[var(--error)] px-2 py-0.5 rounded-md bg-[var(--error)] hover:opacity-80 transition-colors cursor-pointer"
			...
			{loadingContainers.value && list.length === 0 && <div className="text-xs text-[var(--muted)]">Loading&hellip;</div>}
			{!loadingContainers.value && list.length === 0 && <div className="text-xs text-[var(--muted)]" style={{ padding: "4px 0" }}>No containers found.</div>}
			{list.map((c) => <ContainerRow key={c.name} container={c} sandboxAvailable={sandboxAvailable} />)}
		</div>
	);
}
...
function backendRecommendation(info: SandboxInfoValue | null): { level: string; text: string; link?: string } | null {
	...
	if (backend === "none") {
		if (os === "macos") {
			return { level: "warn", text: "No container runtime detected. Install Apple Container (macOS 26+) for VM-isolated sandboxing, or install Docker as an alternative.", link: "https://developer.apple.com/documentation/virtualization" };
		}
		if (os === "linux") {
			return { level: "warn", text: "No container runtime detected. Install Docker for sandboxed execution, or ensure systemd is available for cgroup isolation." };
		}
		return { level: "warn", text: "No container runtime detected. Install Docker for sandboxed execution." };
	}

	if (os === "macos" && backend === "docker") {
		return { level: "info", text: "Apple Container provides stronger VM-level isolation on macOS 26+. Install it for automatic use (moltis prefers it over Docker). Run: brew install container" };
	}
	if (os === "linux" && backend === "docker") {
		return { level: "info", text: "Docker is a good choice on Linux. For lighter-weight isolation without Docker overhead, systemd cgroup sandboxing is also supported." };
	}
	if (backend === "restricted-host") {
		return { level: "info", text: "Using restricted host execution (env clearing, rlimits). For stronger isolation, install Docker or Apple Container." };
	}
	if (backend === "wasm") {
		return { level: "info", text: "Using WASM sandbox with filesystem isolation. For container-level isolation, install Docker or Apple Container." };
	}
		...
		<div className="max-w-form">
			<div className="info-bar" style={{ marginBottom: "8px" }}>
				<span className="info-field">
					...
					<span className="info-value-strong" style={{ color: badgeColor, fontFamily: "var(--font-mono)" }}>{label}</span>
				</span>
			</div>
			{rec && (
...
function DefaultImageSelector(): VNode {
	...
	function onSave(): void {
		...
		fetch("/api/images/default", {
			...
			.catch(() => { /* ignore */ })
			.finally(() => { savingDefault.value = false; });
	}

	return (
		<div className="max-w-form">
			<h3 className="text-sm font-medium text-[var(--text-strong)]" style={{ marginBottom: "8px" }}>Default image</h3>
			...
			<div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
				<input type="text" className="provider-key-input" list="default-image-list"
					placeholder="ubuntu:25.10"
					style={{ flex: 1, fontFamily: "var(--font-mono)", fontSize: ".8rem" }}
					value={current}
					onInput={(e) => { defaultImage.value = (e.target as HTMLInputElement).value; }} />
				<button className="provider-btn" onClick={onSave} disabled={savingDefault.value || !sandboxAvailable}
					title={sandboxAvailable ? undefined : SANDBOX_DISABLED_HINT}>
					{savingDefault.value ? "Saving\u2026" : "Save"}
				</button>
			</div>
			<datalist id="default-image-list">
				{images.value.map((img) => <option key={img.tag} value={img.tag} />)}
			</datalist>
		</div>
	);
}
...
function SharedHomeSection(): VNode {
	...
	return (
		<div className="max-w-form" style={{ borderTop: "1px solid var(--border)", paddingTop: "16px" }}>
			<h3 className="text-sm font-medium text-[var(--text-strong)]" style={{ marginBottom: "8px" }}>Shared home folder</h3>
			...
			{sharedHomeLoading.value
				...
				: (
					<div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
						<label htmlFor="sandboxSharedHomeEnabled" className="text-xs text-[var(--text)]" style={{ display: "flex", alignItems: "center", gap: "8px" }}>
							<input
								id="sandboxSharedHomeEnabled"
								type="checkbox"
								checked={sharedHomeEnabled.value}
								onInput={(e) => { sharedHomeEnabled.value = (e.target as HTMLInputElement).checked; }}
							/>
							<span>Enable shared home folder</span>
						</label>
						<label htmlFor="sandboxSharedHomePath" className="text-xs text-[var(--muted)]">Shared folder location</label>
						<input
							...
							onInput={(e) => { sharedHomePath.value = (e.target as HTMLInputElement).value; }}
							style={{ fontFamily: "var(--font-mono)", fontSize: ".75rem" }}
						/>
						{sharedHomeConfiguredPath.value
							? <div className="text-xs text-[var(--muted)]">Configured path: <code>{sharedHomeConfiguredPath.value}</code></div>
							: <div className="text-xs text-[var(--muted)]">Configured path: <em>default</em></div>
						}
						<div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
							...
							{sharedHomeErr.value
								? <span className="text-xs" style={{ color: "var(--error)" }}>{sharedHomeErr.value}</span>
								: sharedHomeMsg.value
									? <span className="text-xs" style={{ color: "var(--accent)" }}>{sharedHomeMsg.value}</span>
									: null
							}
						</div>
					</div>
		...
		<div className="provider-item" style={{ marginBottom: "4px" }}>
			<div style={{ flex: 1, minWidth: 0 }}>
				<div className="flex items-center gap-2">
					<span className="provider-item-name" style={{ fontFamily: "var(--font-mono)", fontSize: ".8rem" }} title={img.tag}>{truncateHash(img.tag)}</span>
					<span className="text-[0.65rem] px-1.5 py-0.5 rounded-full" style={{ background: `color-mix(in srgb, ${kindColor} 15%, transparent)`, color: kindColor }}>{kindLabel}</span>
				</div>
			...
			<button className="session-action-btn session-delete" title={sandboxAvailable ? "Delete image" : SANDBOX_DISABLED_HINT}
				disabled={!sandboxAvailable}
	...
	return (
		<div className="flex-1 flex flex-col min-w-0 p-4 gap-4 overflow-y-auto">
			{!sandboxRuntimeAvailable() && (
				<div className="alert-warning-text max-w-form"><span className="alert-label-warn">Warning: </span>{SANDBOX_DISABLED_HINT}</div>
			)}
			<div className="flex items-center gap-3">
				<h2 className="text-lg font-medium text-[var(--text-strong)]">Sandboxes</h2>
				<button className="text-xs text-[var(--muted)] border border-[var(--border)] px-2.5 py-1 rounded-md hover:text-[var(--text)] hover:border-[var(--border-strong)] transition-colors cursor-pointer bg-transparent"
			...
			<p className="text-sm text-[var(--muted)] leading-relaxed max-w-form" style={{ margin: 0 }}>
				...
				{sbInfo?.backend === "apple-container" && (
					<>
						<br /><br />Apple Container provides VM-isolated execution but does not support building images. Docker (or OrbStack) is required alongside Apple Container to build and cache custom images. Sandboxed commands run via Apple Container; image builds use Docker.
					</>
				)}
			</p>
			...
			<div className="max-w-form">
				...
				{!loading.value && images.value.length === 0 && <div className="text-xs text-[var(--muted)]" style={{ padding: "12px 0" }}>No cached images.</div>}
				{images.value.map((img) => <ImageRow key={img.tag} image={img} sandboxAvailable={sandboxRuntimeAvailable()} />)}
			</div>

			{/* Build custom image */}
			<div className="max-w-form" style={{ marginTop: "8px", borderTop: "1px solid var(--border)", paddingTop: "16px" }}>
				<h3 className="text-sm font-medium text-[var(--text-strong)]" style={{ marginBottom: "12px" }}>Build custom image</h3>
				<div className="project-edit-group" style={{ marginBottom: "8px" }}>
					<div className="text-xs text-[var(--muted)]" style={{ marginBottom: "4px" }}>Image name</div>
					<input type="text" className="provider-key-input" placeholder="my-tools"
						style={{ width: "100%" }} value={buildName.value}
						onInput={(e) => { buildName.value = (e.target as HTMLInputElement).value; }} />
				</div>
				<div className="project-edit-group" style={{ marginBottom: "8px" }}>
					<div className="text-xs text-[var(--muted)]" style={{ marginBottom: "4px" }}>Base image</div>
					<input type="text" className="provider-key-input" placeholder="ubuntu:25.10"
						style={{ width: "100%", fontFamily: "var(--font-mono)" }} value={buildBase.value}
						onInput={(e) => { buildBase.value = (e.target as HTMLInputElement).value; }} />
				</div>
				<div className="project-edit-group" style={{ marginBottom: "8px" }}>
					<div className="text-xs text-[var(--muted)]" style={{ marginBottom: "4px" }}>Packages (space or newline separated)</div>
					<textarea className="provider-key-input"
						placeholder="ffmpeg python3-pip curl"
						style={{ width: "100%", minHeight: "60px", resize: "vertical", fontFamily: "var(--font-mono)", fontSize: ".8rem" }}
						value={buildPackages.value}
						onInput={(e) => { buildPackages.value = (e.target as HTMLTextAreaElement).value; }} />
				</div>
				<button className="provider-btn" onClick={buildImage}
				...
				{buildWarning.value && (
					<div className="alert-warning-text" style={{ marginTop: "8px" }}>
						<span className="alert-label-warn">Warning: </span>{buildWarning.value}
					</div>
				)}
				{buildStatus.value && (
					buildStatus.value.startsWith("Error")
						? <div className="alert-error-text" style={{ marginTop: "8px" }}><pre>{buildStatus.value}</pre></div>
						: <div className="text-xs" style={{ marginTop: "8px", color: "var(--muted)" }}>{buildStatus.value}</div>
				)}
			</div>
		</div>
	);
