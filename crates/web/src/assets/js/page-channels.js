...
function AddMatrixModal() {
	...
	return html`<${Modal} show=${showAddMatrix.value} onClose=${() => {
	    ...
	    <div class="channel-form">
	      ...
	      ${error.value && html`<div class="text-xs text-[var(--error)] py-1">${error.value}</div>`}
	      <button class="provider-btn" onClick=${onSubmit} disabled=${saving.value || oidcWaiting.value}>${saving.value ? "Connecting\u2026" : oidcWaiting.value ? "Waiting for OIDC\u2026" : authModeDraft.value === "oidc" ? "Authenticate with OIDC" : "Connect Matrix"}</button>
	    </div>
	  </${Modal}>`;
}
