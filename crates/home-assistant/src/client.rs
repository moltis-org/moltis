...
impl HomeAssistantClient {
    ...
    /// Check if the HA instance is reachable.
    ...
    pub async fn call_service(
        ...
        if let Some(d) = data {
            // Merge service_data fields into the top-level body.
            // HA's REST endpoint expects the raw body as service_data.
    ...
    pub async fn turn_on(&self, entity_id: &str) -> Result<()> {
        ...
        self.call_service(domain, "turn_on", None, Some(data))
            .await?;
        Ok(())
    }
    ...
    pub async fn turn_off(&self, entity_id: &str) -> Result<()> {
        let domain = entity_id
            ...
            .unwrap_or("homeassistant");
        let data = serde_json::json!({ "entity_id": entity_id });
        self.call_service(domain, "turn_off", None, Some(data))
            .await?;
        Ok(())
    }
    ...
    pub async fn toggle(&self, entity_id: &str) -> Result<()> {
        let domain = entity_id
            ...
            .unwrap_or("homeassistant");
        let data = serde_json::json!({ "entity_id": entity_id });
        self.call_service(domain, "toggle", None, Some(data))
            .await?;
        Ok(())
    }
    ...
    pub async fn get_history(
        ...
        let (_, auth) = self.auth_header();
        let mut url = format!(
            "{}/api/history/period/{start_time}?filter_entity_id={filter_entity_id}",
            self.base_url
        );
